...
Available storage capacity and the pricing for storing and retrieving files are not controlled by any single company. Instead, the Filecoin storage network facilitates open markets for storing and retrieving files that anyone can participate in. It uses its native cryptocurrency (FIL) to faciliate these transactions.
