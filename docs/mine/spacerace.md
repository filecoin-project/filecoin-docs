...
To be eligible for Space Race rewards, each miner will need to demonstrate they they can execute _one_ sector upgrade per miner.
...
* Use that sector number to run `./lotus-miner sectors mark-for-upgrade {{sector-number}}`.
