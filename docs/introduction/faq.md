...
If you are retrieving your data from IPFS or a remote pinning layer such as an [FPS](../build/tools/filecoin-backed-pinning-services.md), retrieval should take on the order of milliseconds to seconds in the worst case. Our latest tests for retrieval from the Filecoin network directly show that a sealed sector holding data takes ~1 hour to unseal. 1-5 hours is our best real world estimate to go from sector unsealing to ultimate delivery of the data. If you need faster data retrieval for your application, we recommend building on Powergate or an FPS.
