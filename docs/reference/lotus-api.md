...
Notice how the CID has been serialized (a common pitfall. The cURL request will then be:
...
And the response is a JSON-serialized [Message type](https://pkg.go.dev/github.com/filecoin-project/lotus/chain/types?tab=doc#Message). All the above should help you getting bootstrapped to harness the power of the Lotus APIs!
...
It takes the following params: `<multisig address>`, `<sender address of the propose msg>`, `<old signer>`, `<new signer>`
