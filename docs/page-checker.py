...
htmlToTest = markdownToTest.replace("docs/","docs/.vuepress/dist/build/").replace("_index.md","index.html").replace("index.md","index.html").replace(".md","/index.html")
...
for codeBlock in soup.find_all("pre"):
    codeBlock.decompose()
pageText = soup.find(id="home-container").get_text().replace("&bullet","")
...
for link in links:
    ...
    "filecoin.io" not in linkUrl and
...
disable_rules = [
    ...
    "SENTENCE_WHITESPACE", "DATE_NEW_YEAR", "COMMA_COMPOUND_SENTENCE"
]
