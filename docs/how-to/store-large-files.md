...
The simple recommendation is that files must be smaller than a sector. Colloquially, we refer to sectors as 8MiB, 32GiB, etc. To fully maximize storage usage, chunks can be fractionally larger. This is due to a combination of quirks:

- **Binary size:** Filecoin uses base 2 (binary) sizes, so a 1MiB sector is 1,048,576 bytes not 1,000,000 bytes.
